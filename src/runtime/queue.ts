...
import { handleMarketplaceQueueBatch } from "../queues/marketplace-event.consumer";
...
export async function handleQueueEvent(
    ...
    if (batch.queue === "crm-marketplace-events-dlq") {
        await handleMarketplaceDlqBatch(
            batch as QueueBatchLike<MarketplaceEventQueueMessage>,
            env
        );
        return;
    }
    ...
    if (batch.queue === "crm-marketplace-events") {
