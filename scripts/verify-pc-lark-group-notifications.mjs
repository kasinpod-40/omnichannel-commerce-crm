...
const confirmation =
    confirmationIndex >= 0 ? args[confirmationIndex + 1] : "";
const SEND_CONFIRMATION = "PC-LARK-GROUP-TEST";
const CONTRACT_VERSION = "pc_lark_group_notification_readiness_v2";
...
function assertStaticContracts() {
    ...
    const envTypes = readRequiredFile("src/config/env.ts");
    ...
    const queueRuntime = readRequiredFile("src/runtime/queue.ts");

    assert(
        /"PC_INVENTORY_ENABLED"\s*:\s*"false"/u.test(wrangler),
        "STOP: PC_INVENTORY_ENABLED must remain false during notification readiness"
    );
    assert(
        /"binding"\s*:\s*"NOTIFICATION_QUEUE"/u.test(wrangler) &&
            /"queue"\s*:\s*"crm-notifications"/u.test(wrangler),
        "Missing NOTIFICATION_QUEUE producer binding"
    );
    assert(
        /"dead_letter_queue"\s*:\s*"crm-notifications-dlq"/u.test(wrangler),
        "Missing crm-notifications DLQ contract"
    );
    assert(
        envTypes.includes("LARK_GROUP_WEBHOOK_URL") &&
            envTypes.includes("LARK_PC_GROUP_WEBHOOK_URL"),
        "Env contract must preserve the CRM webhook and add the dedicated PC webhook"
    );
    assert(
        notificationTypes.includes('"PC_STOCK_EXCEPTION"') &&
            notificationTypes.includes('"PC_MATERIAL_SHORTAGE"'),
        "PC notification types are missing"
    );
    assert(
        alerts.includes("recordAndDispatchNotificationOnce") &&
            alerts.includes("notifyPcExceptionOnce"),
        "PC alert dispatch contract is missing"
    );

    const formatterContract =
        notificationService.includes("sendLarkGroupText") &&
        notificationService.includes("enqueueNotificationDelivery") &&
        ...
        notificationService.includes('PC_STOCK_EXCEPTION: "📦"') &&
        notificationService.includes('PC_MATERIAL_SHORTAGE: "🧵"');
    assert(
        formatterContract,
    ...
    assert(
        groupProvider.includes("resolveLarkGroupWebhookTarget") &&
            groupProvider.includes("LARK_PC_GROUP_WEBHOOK_URL") &&
            groupProvider.includes('"production-control"') &&
            groupProvider.includes("📦 พบข้อยกเว้นด้านสต็อกสินค้า") &&
            groupProvider.includes("🧵 วัตถุดิบไม่เพียงพอสำหรับแผนผลิต"),
        "Dedicated PC Lark Group routing is missing"
    );
    assert(
        groupProviderTest.includes("LARK_PC_GROUP_WEBHOOK_URL_NOT_CONFIGURED") &&
            groupProviderTest.includes("ไม่ fallback ไปกลุ่ม CRM เดิม"),
        "Fail-closed PC group routing regression is missing"
    );
    assert(
        queueRuntime.includes("handleNotificationQueueBatch") &&
            queueRuntime.includes('batch.queue === "crm-notifications"'),
    ...
    return {
        ...
        routing: {
            crm_notifications: "LARK_GROUP_WEBHOOK_URL",
            production_stock_notifications: "LARK_PC_GROUP_WEBHOOK_URL",
        },
        dispatch_path:
...
function wranglerBinary() {
    ...
    const binary = path.resolve(process.cwd(), "node_modules", ".bin", name);
    assert(
...
function parseWranglerJson(stdout) {
    ...
    assert(text, "Wrangler secret list returned empty output");
    try {
        return JSON.parse(text);
    } catch {
        const start = Math.max(text.lastIndexOf("["), text.lastIndexOf("{"));
        assert(start >= 0, "Wrangler secret list did not return JSON");
...
function listRemoteSecrets() {
    ...
    const names = secretNamesFromPayload(parseWranglerJson(result.stdout));
    const required = [
    ...
    return {
        ...
        old_crm_group_secret_preserved: names.includes(
            "LARK_GROUP_WEBHOOK_URL"
        ),
        dedicated_pc_group_secret_present: names.includes(
            "LARK_PC_GROUP_WEBHOOK_URL"
        ),
...
function getWebhookKeyword() {
    const keyword = (env.LARK_GROUP_WEBHOOK_KEYWORD || "CRM").trim();
    assert(keyword, "LARK_GROUP_WEBHOOK_KEYWORD must not be empty");
    assert(
        keyword.length <= 80 && !/[\r\n]/u.test(keyword),
...
function responseMessage(payload) {
    ...
    for (const key of ["msg", "StatusMessage", "message"]) {
...
async function sendWebhookTest() {
    assert(
        ...
        `STOP: --send-test requires --confirm-send ${SEND_CONFIRMATION}`
    );
    const webhookUrl = (env.LARK_PC_GROUP_WEBHOOK_URL || "").trim();
    assert(
        webhookUrl,
        `Missing LARK_PC_GROUP_WEBHOOK_URL in ${envFile}. Place the same PC Group Webhook value in the local env file before the one-time test.`
    );
    assert(
    ...
    return {
        ...
        secret_name: "LARK_PC_GROUP_WEBHOOK_URL",
        test_id: testId,
...
function readPriorTestEvidence() {
    ...
    assert(
        parsed?.contract_version === CONTRACT_VERSION &&
            ...
            parsed?.webhook_test?.target === "production-control" &&
            parsed?.webhook_test?.secret_name === "LARK_PC_GROUP_WEBHOOK_URL",
        "Prior evidence is not a successful dedicated PC Group send-test"
    );

    const sentAt = Date.parse(parsed.webhook_test.sent_at || "");
    assert(Number.isFinite(sentAt), "Prior send-test timestamp is invalid");
...
async function main() {
    ...
    const result = {
        ...
        safety: {
            ...
            group_test_messages_sent: mode === "send-test" ? 1 : 0,
        },
...
main().catch((error) => {
    console.error(`FAILED: ${redact(error?.stack || error)}`);
    process.exitCode = 1;
});
