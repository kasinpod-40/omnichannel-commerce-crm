...
function assertStaticContracts() {
    ...
    assert(
        /"PC_INVENTORY_ENABLED"\s*:\s*"false"/.test(wrangler),
        "STOP: PC_INVENTORY_ENABLED must remain false during notification readiness"
    );
    assert(
        /"binding"\s*:\s*"NOTIFICATION_QUEUE"/.test(wrangler) &&
            /"queue"\s*:\s*"crm-notifications"/.test(wrangler),
        "Missing NOTIFICATION_QUEUE producer binding"
    );
    assert(
        /"queue"\s*:\s*"crm-notifications"/.test(wrangler) &&
            /"dead_letter_queue"\s*:\s*"crm-notifications-dlq"/.test(
                wrangler
            ),
    ...
    assert(
        ...
        "Env contract is missing LARK_GROUP_WEBHOOK_URL"
    );
    assert(
    ...
    assert(
        ...
        "Notification queue/webhook delivery contract is missing"
    );
    assert(
        groupProvider.includes("LARK_GROUP_WEBHOOK_URL") &&
            groupProvider.includes("LARK_GROUP_WEBHOOK_KEYWORD"),
        "Lark Group Webhook provider contract is missing"
    );
    assert(
    ...
    return {
        ...
        dispatch_path:
            "PC alert -> Notifications table -> crm-notifications -> Lark Group Webhook",
    };
}
...
function listRemoteSecrets() {
    ...
    const required = ["LARK_GROUP_WEBHOOK_URL"];
...
async function sendWebhookTest() {
    ...
    assert(
        webhookUrl,
        `Missing LARK_GROUP_WEBHOOK_URL in ${envFile}. Cloudflare secret values cannot be read back; place the same value in the local env file before the one-time test.`
    );
    assert(
    ...
    const text = [
        `[${keyword}] 🧪 ทดสอบการแจ้งเตือน Production & Stock`,
        "",
        `รหัสทดสอบ: ${testId}`,
        "สถานะ: เชื่อมต่อ Lark Group Webhook สำเร็จ",
        "ขอบเขต: ส่งข้อความทดสอบเท่านั้น",
    ...
    assert(
        response.ok,
        `Lark Group Webhook HTTP ${response.status}: ${String(raw).slice(0, 500)}`
    );

    const code = responseCode(payload);
    assert(
        code === null || code === 0,
        `Lark Group Webhook rejected test (${code}): ${responseMessage(payload)}`
    );
...
function readPriorTestEvidence() {
    ...
    assert(
        parsed?.mode === "send-test" && parsed?.webhook_test?.ok === true,
        "Prior evidence is not a successful send-test"
    );
...
async function main() {
    ...
    const priorTest = mode === "verify" ? readPriorTestEvidence() : null;
    const webhookTest = mode === "send-test" ? await sendWebhookTest() : priorTest;

    const result = {
        contract_version: "pc_lark_group_notification_readiness_v1",
        mode,
